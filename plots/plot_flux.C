...
void plot_flux(TString fluxname, Int_t titleopt)
{
	...
	TGraph * gr0= new TGraph(numpoints-1,en,nuebar);
	gr0->SetLineWidth(4);

	gr0->Draw("same");
	  //  gr0->Print();

	TGraph * gr1= new TGraph(numpoints-1,en,nue);
	gr1->SetLineColor(2);
	gr1->SetLineWidth(4);

	gr1->Draw("same");

	TGraph * gr2= new TGraph(numpoints-1,en,nux);
	gr2->SetLineColor(3);
	gr2->SetLineWidth(4);

	gr2->Draw("same");

	TLegend * leg = new TLegend(0.45,0.7,0.82,0.85);
